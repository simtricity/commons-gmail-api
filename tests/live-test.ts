...
const gmail = await GmailClient.fromStore({ clientSecret, store: ctx.store, account: ctx.account });

Deno.test("profile resolves to the stored account", async () => {
  const p = await gmail.profile();
  assertEquals(p.emailAddress.toLowerCase(), (await gmail.email()).toLowerCase());
});

Deno.test("a recent message with an attachment round-trips through fetch + manifest", async () => {
  const page = await gmail.listMessages("has:attachment newer_than:30d smaller:2M", { maxResults: 1 });
  const hit = page.messages?.[0];
  if (!hit) throw new Error("no recent small message with an attachment in this mailbox");
  const dir = await Deno.makeTempDir({ prefix: "gmail-api-live-" });
  try {
    const manifest = await fetchThreadAttachments(gmail, hit.threadId, { outDir: dir });
